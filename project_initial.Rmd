...
Any type of model/equation can be used as a predictive model. To use a model as a predictive model, you must train and test the model on a dataset, and then use it to predict future data.

### Training and Testing sets

Typically, when creating a predictive model, you split your data into a training and testing set. Then, you create the model using the training set data, and test the model on the testing set, to see how accurate the model is in predicting data that wasn't used to create it.
...
Both methods measure the accuracy of a model, particularly how far away each point is from the predicted value(s). The only difference is one is the square root and the other divides by the total number of data points.  
...
Decision trees are used for categorical target, whereas regression is used for a numerical target. Decision trees are a type of classification model. The accuracy of decision trees are measured using a confusion matrix, which is essentially a truth table. This plays the same role as the MSE/RMSE for regression.  
